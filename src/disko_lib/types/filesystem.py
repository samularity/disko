...
from disko_lib.logging import DiskoMessage, debug
from disko_lib.messages.msgs import err_filesystem_changed_no_destroy
from .device import BlockDevice
from ..result import DiskoResult, DiskoSuccess, DiskoError
...
def generate_plan(
    ...
    if need_to_destroy_current:
        plan.append(
            Step(
                "destroy",
                [[f"mkfs.{format}"] + target_config.extraArgs + [device]],
