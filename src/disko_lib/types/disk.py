...
def generate_plan(
    ...
    for name, target_disk_config in target_disks.items():
        ...
        disk_has_same_type = (
            current_disk_config is not None
            and current_disk_config.type != target_disk_config.type
        )

        debug(f"Disk '{name}': {disk_exists=}, {disk_has_same_type=}")
        ...
        debug(f"Disk '{name}': {current_disk_config.type=}, {target_disk_config.type}")
